...
function Complete-DbTransaction {
    ...
    process {
        ...
        if ($sqlCommand.Connection.State -ne "Open") {
            Write-Error "SqlCommand must be open before completing a transaction"
        }
        if ($null -eq $SqlCommand.Transaction) {
            Write-Error "SqlCommand must be associated with a transaction before completing a transaction"
        }

        if ($TransactionName) {
            $SqlCommand.Transaction.Commit($TransactionName)
        } else {
